...
layers3 = [imageInputLayer([28 28 1])
...
layers4 = [imageInputLayer([28 28 1])
...
layers7 = [imageInputLayer([28 28 1])
  convolution2dLayer(3,12)
  reluLayer
...
layers8 = [imageInputLayer([28 28 1])
  convolution2dLayer(5,12)
  reluLayer 
...
layers9 = [imageInputLayer([28 28 1])
  convolution2dLayer(5,24)
  reluLayer 
...
layers10 = [imageInputLayer([28 28 1])
  convolution2dLayer(5,12)
  reluLayer 
...
convnet = cell(1,length(layers));
for l=1:length(layers)
    tic
    figure
    convnet{l} = trainNetwork(trainDigitData,layers{l},options);
    toc
end
...
YTest = cell(1,length(layers));
TTest = YTest;
for n=1:length(convnet)
    YTest{n} = classify(convnet{n},testDigitData);
    TTest{n} = testDigitData.Labels;
end

%% 
% Calculate the accuracy. 
accuracy = zeros(1,length(layers));
for n=1:length(layers)
    accuracy(n) = sum(YTest{n} == TTest{n})/numel(TTest{n});
end
